...
* **Synchronization (vs Linux)**: ``WaitOnAddress`` / ``WakeByAddress*`` pair on **the same virtual address**. Each ``MapViewOfFile`` of a section uses a different VA, and different processes map different VAs, so those primitives do **not** reliably wake waiters across two channel attachments or across processes. xproc therefore implements ``atomic_wait`` on Windows as a **spin / yield / timed ``WaitOnAddress`` loop**: same-process waiters wake promptly via ``WakeByAddress*``, hot cross-process shared-memory hand-offs usually complete before the timed wait, and idle cross-process waiters keep making progress by timing out and re-checking the sequence word. Linux uses the futex, which coordinates correctly on shared mappings.
