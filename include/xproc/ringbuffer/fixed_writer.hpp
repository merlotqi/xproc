...
class fixed_writer : public ringbuffer_view {
 public:
  ...
  void* reserve(uint32_t item_size, uint64_t& out_pos) {
    const uint32_t total_len = align_size(item_size + sizeof(detail::fixed_message_header));
    while (true) {
      ...
      if (header_->rb_meta.write_pos.compare_exchange_strong(curr_write, curr_write + total_len)) {
        out_pos = curr_write;
        auto* h = reinterpret_cast<detail::fixed_message_header*>(get_ptr(out_pos));
        h->status.store(0, std::memory_order_relaxed);
        return get_ptr(out_pos + sizeof(detail::fixed_message_header));
      }
    }
  }
