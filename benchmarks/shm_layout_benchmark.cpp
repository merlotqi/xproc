...
void init_valid_header(xproc::shm::shm_control_block& h, std::uint64_t cap, std::uint32_t layout_type,
...
static void BM_ShmLayoutValidate(benchmark::State& state) {
  ...
  for (auto _ : state) {
    const bool ok = xproc::shm::shm_layout_manager::validate(hdr, expected_cap, layout_type, 8u);
